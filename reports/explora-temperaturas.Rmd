...
```{r}
climas %>%
    filter(ano > 1992 & chuva > 0) %>%
        ggplot(mapping = aes(x = tmedia, y = chuva, color = cidade)) +
        geom_jitter(width = .3) +
        geom_smooth(aes(x = tmedia, y = chuva),method = lm)
...
No gráfico, é possível inferir que nos meses entre Setembro e Dezembro existe uma similaridade entre a quantidade de chuva recebida por ambas as cidades. Em contra partida, entre os meses de Março e Junho, a cidade de João Pessoa recebe uma quantidade maior de chuvas com mais de 50mm.

```{r}
climas %>%
    group_by(ano, mes, cidade) %>%
    summarise(chuva_periodo = median(chuva)) %>%
    ggplot(aes(x= mes,y = chuva_periodo, colour= cidade)) + 
    geom_jitter() +
    scale_x_continuous(breaks = seq(1,12)) +
    labs(x = "Mês", y = "Quantidade de Chuva (em mm)",colour = "Cidade")
```
